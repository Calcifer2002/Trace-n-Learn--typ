...
public class SCustomView extends View {
    ...
    // To set stroke color that we get from S_Activity color panel, so we pass the index into this
...
//Different accuracy messages for different situations
        if (strokeCount <= 2 && totalStrokeCoordinates > 120 && accuracy > 90) {
            return "Accuracy Score: " + accuracy + "%"; //Accurate letter, max 2 strokes to achieve S
