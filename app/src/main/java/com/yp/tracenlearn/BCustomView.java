...
public class BCustomView extends View {
    ...
    private int strokeColor = Color.BLACK; //Default stroke colour
    ...
    private BCustomView.NoStrokesCallback noStrokesCallback;
    ...
    //When the customview begins we initialise it with default stroke properties and code to help keep track of movement
    private void init() {
        mPath = new Path();
        mPaint = new Paint();
    ...
    //To set stroke colour that we get from B_Activity colour panel so we pass the index into this
    ...
    /*
    When size of view changes we need to redo bitmap accordingly, so we put it in canvas and center it after scaling
    ...
    */
    ...
    public void setOnNoStrokesDetectedCallback(BCustomView.NoStrokesCallback callback) {
    ...
    */
    private String getAccuracyInfo() {
        ...
        if (strokeCount <= 3 && totalStrokeCoordinates > 130 && accuracy > 90) {
            return "Accuracy Score: " + accuracy + "%"; //Accurate letter, max 3 strokes to achieve B

        } else if (strokeCount > 3) {
            return "NO: " + accuracy + "%" + "many"; //If letter is drawn with way too many strokes
        } else if (totalStrokeCoordinates < 130 && accuracy > 90) {
