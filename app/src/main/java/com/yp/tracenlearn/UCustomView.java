...
public class UCustomView extends View {
    ...
    private UCustomView.NoStrokesCallback noStrokesCallback;
    private Handler handler = new Handler(); // To handle delays

    public UCustomView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    // To set stroke color that we get from U_Activity color panel, so we pass the index into this
    ...
    */
    ...
    protected void onSizeChanged(int w,int h, int oldw, int oldh) {
        ...
        BitmapDrawable drawable = (BitmapDrawable) getResources().getDrawable(R.drawable.letter_u);
    ...
    public void setOnNoStrokesDetectedCallback(UCustomView.NoStrokesCallback callback) {
...
//Different accuracy messages for different situations
        if (strokeCount <= 2 && totalStrokeCoordinates > 120 && accuracy > 90) {
            return "Accuracy Score: " + accuracy + "%"; //Accurate letter, max 2 strokes to achieve U
