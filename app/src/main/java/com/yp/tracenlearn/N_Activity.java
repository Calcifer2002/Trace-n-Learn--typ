...
public class N_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MediaPlayer mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.n);
        ...
        dialogNoMany = new Dialog(N_Activity.this);
        dialogNoSlow = new Dialog(N_Activity.this);
                    ...
                    if (child instanceof ImageView) {
                        ...
                        child.setOnClickListener(new View.OnClickListener() { // We wait for click
                            ...
                            public void onClick(View v) {
                                customNCanvas.setStrokeColor(getColorForIndex(colorIndex)); // Get the respective colour for that index
                            }
                        });
                    }
                }
        ...
        customNCanvas.setOnNoStrokesDetectedCallback(new NCustomView.NoStrokesCallback() {
            ...
            public void onNoStrokesDetected(String accuracyInfo) {
                if (freePlay && accuracyInfo.toLowerCase().contains("no")) {     //When in freeplaymode and the letter is not traced properly
                    ...
                    databaseReference.child("users").child(uid).child("n-freeplay-incorrect").setValue(rated); //Keeping track of wrong letter rate
                    databaseReference.child("users").child(uid).child("n-freeplay-flower").setValue(0); //To track flower counter- wrong trace so we set it to 0
                    databaseReference.child("users").child(uid).child("n-freeplay").setValue("0");
                    ...
                    String rate = accuracyInfo.substring(colonIndex + 1, percentIndex).trim();
                    float rated = Float.parseFloat(rate);
                    databaseReference.child("users").child(uid).child("n-freeplay-correct").setValue(rated);//Keeping track of correct letter rate
                    databaseReference.child("users").child(uid).child("n-freeplay-flower").setValue(1); //To track flower counter- correct trace so we set it to 1
                    databaseReference.child("users").child(uid).child("n-freeplay").setValue("1");
                    ...
                    new Handler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            databaseReference.child("users").child(uid).child("n-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("n-attempted").setValue(1);
                    ...
                    new Handler().postDelayed(new Runnable() {  //Same as previous
                        ...
                        public void run() {
                            ...
                            float rated = Float.parseFloat(rate);
                            databaseReference.child("users").child(uid).child("n-incorrect").setValue(rated);//Updating incorrect counters
                            databaseReference.child("users").child(uid).child("n-flower").setValue(0);
                            databaseReference.child("users").child(uid).child("n-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("n-attempted").setValue(1);
                            Intent intent = new Intent(N_Activity.this, N_Activity.class);
                    ...
                    databaseReference.child("users").child(uid).child("n-flower").setValue(1);
                    databaseReference.child("users").child(uid).child("n-freeplay").setValue("1");
                    databaseReference.child("users").child(uid).child("n-attempted").setValue(1);//We save the accuracy rate for that letter in the db
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            dialogYes.dismiss(); //Same logic as above we show dialog for 6 seconds

                            Intent intent = new Intent(N_Activity.this, Difficult_Activity.class);
                            startActivity(intent);
                            finish();
                        }
                    }, 6000);
