...
public class E_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MediaPlayer mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.e);
                    ...
                    if (child instanceof ImageView) {
                        ...
                        child.setOnClickListener(new View.OnClickListener() { // We wait for click
                            ...
                            public void onClick(View v) {
                                customECanvas.setStrokeColor(getColorForIndex(colorIndex)); // Get the respective colour for that index
                            }
                        });
                    }
                }
        ...
        customECanvas.setOnNoStrokesDetectedCallback(new ECustomView.NoStrokesCallback() {
            ...
            public void onNoStrokesDetected(String accuracyInfo) {
                if (freePlay && accuracyInfo.toLowerCase().contains("no")) {     //When in freeplaymode and the letter is not traced properly
                    ...
                    databaseReference.child("users").child(uid).child("e-freeplay-incorrect").setValue(rated); //Keeping track of wrong letter rate
                    databaseReference.child("users").child(uid).child("e-freeplay-flower").setValue(0); //To track flower counter- wrong trace so we set it to 0
                    ...
                    String rate = accuracyInfo.substring(colonIndex + 1, percentIndex).trim();
                    float rated = Float.parseFloat(rate);
                    databaseReference.child("users").child(uid).child("e-freeplay-correct").setValue(rated);//Keeping track of correct letter rate
                    databaseReference.child("users").child(uid).child("e-freeplay-flower").setValue(1); //To track flower counter- correct trace so we set it to 1
                    ...
                    new Handler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            databaseReference.child("users").child(uid).child("e-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("e-attempted").setValue(1);
                    ...
                    new Handler().postDelayed(new Runnable() {  //Same as previous
                        ...
                        public void run() {
                            ...
                            float rated = Float.parseFloat(rate);
                            databaseReference.child("users").child(uid).child("e-incorrect").setValue(rated);//Updating incorrect counters
                            databaseReference.child("users").child(uid).child("e-flower").setValue(0);
                            databaseReference.child("users").child(uid).child("e-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("e-attempted").setValue(1);
                            Intent intent = new Intent(E_Activity.this, E_Activity.class);
                    ...
                    databaseReference.child("users").child(uid).child("e-freeplay").setValue("1");
                    databaseReference.child("users").child(uid).child("e-attempted").setValue(1);//We save the accuracy rate for that letter in the db
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
