...
public class M_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MCustomView customMCanvas = findViewById(R.id.customMCanvas); // Drawing canvas
        ...
        dialogNo = new Dialog(M_Activity.this);
        dialogYes = new Dialog(M_Activity.this);
        dialogNoMany = new Dialog(M_Activity.this);
        dialogNoSlow = new Dialog(M_Activity.this);
                    ...
                    if (child instanceof ImageView) {
                        ...
                        child.setOnClickListener(new View.OnClickListener() { // We wait for click
                            ...
                            public void onClick(View v) {
                                customMCanvas.setStrokeColor(getColorForIndex(colorIndex)); // Get the respective colour for that index
                            }
                        });
                    }
                }
        ...
        */
        customMCanvas.setOnNoStrokesDetectedCallback(new MCustomView.NoStrokesCallback() {
            @Override
            public void onNoStrokesDetected(String accuracyInfo) {
                if (freePlay && accuracyInfo.toLowerCase().contains("no")) {     //When in freeplaymode and the letter is not traced properly
                    ...
                    databaseReference.child("users").child(uid).child("m-freeplay-incorrect").setValue(rated); //Keeping track of wrong letter rate
                    databaseReference.child("users").child(uid).child("m-freeplay-flower").setValue(0); //To track flower counter- wrong trace so we set it to 0
                    databaseReference.child("users").child(uid).child("m-freeplay").setValue("0");
                    ...
                    String rate = accuracyInfo.substring(colonIndex + 1, percentIndex).trim();
                    float rated = Float.parseFloat(rate);
                    databaseReference.child("users").child(uid).child("m-freeplay-correct").setValue(rated);//Keeping track of correct letter rate
                    databaseReference.child("users").child(uid).child("m-freeplay-flower").setValue(1); //To track flower counter- correct trace so we set it to 1
                    databaseReference.child("users").child(uid).child("m-freeplay").setValue("1");
                    ...
                    new Handler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            float rated = Float.parseFloat(rate);
                            databaseReference.child("users").child(uid).child("m-incorrect").setValue(rated);
                            databaseReference.child("users").child(uid).child("m-flower").setValue(0);
                            databaseReference.child("users").child(uid).child("m-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("m-attempted").setValue(1);
                            Intent intent = new Intent(M_Activity.this, M_Activity.class);
                    ...
                    new Handler().postDelayed(new Runnable() {  //Same as previous
                        ...
                        public void run() {
                            ...
                            float rated = Float.parseFloat(rate);
                            databaseReference.child("users").child(uid).child("m-incorrect").setValue(rated);//Updating incorrect counters
                            databaseReference.child("users").child(uid).child("m-flower").setValue(0);
                            databaseReference.child("users").child(uid).child("m-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("m-attempted").setValue(1);
                            Intent intent = new Intent(M_Activity.this, M_Activity.class);
                    ...
                    databaseReference.child("users").child(uid).child("m").setValue(rated);
                    databaseReference.child("users").child(uid).child("m-flower").setValue(1);
                    databaseReference.child("users").child(uid).child("m-freeplay").setValue("1");
                    databaseReference.child("users").child(uid).child("m-attempted").setValue(1);//We save the accuracy rate for that letter in the db
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            dialogYes.dismiss(); //Same logic as above we show dialog for 6 seconds

                            Intent intent = new Intent(M_Activity.this, Difficult_Activity.class);
                            startActivity(intent);
                            finish();
                        }
                    }, 6000);
