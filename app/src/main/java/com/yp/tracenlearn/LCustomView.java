...
public class LCustomView extends View {
    ...
    private LCustomView.NoStrokesCallback noStrokesCallback;
    private Handler handler = new Handler(); // To handle delays

    public LCustomView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    // To set stroke color that we get from L_Activity color panel so we pass the index into this
    ...
    */
    ...
    protected void onSizeChanged(int w,int h, int oldw, int oldh) {
        ...
        BitmapDrawable drawable = (BitmapDrawable) getResources().getDrawable(R.drawable.letter_l);
    ...
    public void setOnNoStrokesDetectedCallback(LCustomView.NoStrokesCallback callback) {
    ...
    */
    private String getAccuracyInfo() {
        ...
        if (strokeCount <= 3 && totalStrokeCoordinates > 90 && accuracy > 90) {
            return "Accuracy Score: " + accuracy + "%"; //Accurate letter, max 3 strokes to achieve L

        } else if (strokeCount > 3) {
            return "NO: " + accuracy + "%" + "many"; //If letter is drawn with way too many strokes
        } else if (totalStrokeCoordinates < 90 && accuracy > 90) {
