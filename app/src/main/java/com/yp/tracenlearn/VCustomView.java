...
public class VCustomView extends View {
    ...
    private VCustomView.NoStrokesCallback noStrokesCallback;
    private Handler handler = new Handler(); // To handle delays

    public VCustomView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    // To set stroke color that we get from V_Activity color panel, so we pass the index into this
    ...
    */
    ...
    protected void onSizeChanged(int w,int h, int oldw, int oldh) {
        ...
        BitmapDrawable drawable = (BitmapDrawable) getResources().getDrawable(R.drawable.letter_v);
    ...
    public void setOnNoStrokesDetectedCallback(VCustomView.NoStrokesCallback callback) {
    ...
    */
    private String getAccuracyInfo() {
        ...
        if (strokeCount <= 3 && totalStrokeCoordinates > 80 && accuracy > 90) {
            return "Accuracy Score: " + accuracy + "%"; //Accurate letter, max 2 strokes to achieve V

        } else if (strokeCount > 3) {
            return "NO: " + accuracy + "%" + "many"; //If letter is drawn with way too many strokes
        } else if (totalStrokeCoordinates < 180 && accuracy > 90) {
