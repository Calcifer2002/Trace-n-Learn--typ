...
public class QCustomView extends View {
    ...
    private QCustomView.NoStrokesCallback noStrokesCallback;
    private Handler handler = new Handler(); // To handle delays

    public QCustomView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    // To set stroke color that we get from Q_Activity color panel, so we pass the index into this
    ...
    */
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        BitmapDrawable drawable = (BitmapDrawable) getResources().getDrawable(R.drawable.letter_q);
    ...
    public void setOnNoStrokesDetectedCallback(QCustomView.NoStrokesCallback callback) {
...
//Different accuracy messages for different situations
        if (strokeCount <= 3 && totalStrokeCoordinates > 130 && accuracy > 90) {
            return "Accuracy Score: " + accuracy + "%"; //Accurate letter, max 3 strokes to achieve Q
