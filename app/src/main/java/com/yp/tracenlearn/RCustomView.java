...
public class RCustomView extends View {
    ...
    private RCustomView.NoStrokesCallback noStrokesCallback;
    private Handler handler = new Handler(); // To handle delays

    public RCustomView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    // To set stroke color that we get from R_Activity color panel, so we pass the index into this
    ...
    */
    ...
    protected void onSizeChanged(int w,int h, int oldw, int oldh) {
        ...
        BitmapDrawable drawable = (BitmapDrawable) getResources().getDrawable(R.drawable.letter_r);
    ...
    public void setOnNoStrokesDetectedCallback(RCustomView.NoStrokesCallback callback) {
...
//Different accuracy messages for different situations
        if (strokeCount <= 3 && totalStrokeCoordinates > 140 && accuracy > 90) {
            return "Accuracy Score: " + accuracy + "%"; //Accurate letter, max 3 strokes to achieve R
