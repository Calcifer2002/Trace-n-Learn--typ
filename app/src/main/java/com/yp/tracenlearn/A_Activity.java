...
public class A_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ACustomView customACanvas = findViewById(R.id.customACanvas); // Drawing canvas
        ...
        dialogNo = new Dialog(A_Activity.this);
        dialogYes = new Dialog(A_Activity.this);
        dialogNoMany = new Dialog(A_Activity.this);
        dialogNoSlow = new Dialog(A_Activity.this);
 ...
 */
                    ...
                    if (child instanceof ImageView) {
                        ...
                        child.setOnClickListener(new View.OnClickListener() { //We wait for click
                            ...
                            public void onClick(View v) {
                                customACanvas.setStrokeColor(getColorForIndex(colorIndex)); // Get the respective colour for that index
                            }
                        });
                    }
                }
        ...
        */
        customACanvas.setOnNoStrokesDetectedCallback(new ACustomView.NoStrokesCallback() {
            @Override
            public void onNoStrokesDetected(String accuracyInfo) {
                if (freePlay && accuracyInfo.toLowerCase().contains("no")) {     //When in freeplaymode and the letter is not traced properly
                    ...
                    databaseReference.child("users").child(uid).child("a-freeplay-incorrect").setValue(rated); //Keeping track of wrong letter rate
                    databaseReference.child("users").child(uid).child("a-freeplay-flower").setValue(0); //To track flower counter- wrong trace so we set it to 0
                    ...
                    String rate = accuracyInfo.substring(colonIndex + 1, percentIndex).trim();
                    float rated = Float.parseFloat(rate);
                    databaseReference.child("users").child(uid).child("a-freeplay-correct").setValue(rated);//Keeping track of correct letter rate
                    databaseReference.child("users").child(uid).child("a-freeplay-flower").setValue(1); //To track flower counter- correct trace so we set it to 1
                    databaseReference.child("users").child(uid).child("a-freeplay").setValue("1");
                    ...
                    new Handler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            float rated = Float.parseFloat(rate);
                            databaseReference.child("users").child(uid).child("a-incorrect").setValue(rated);
                            databaseReference.child("users").child(uid).child("a-flower").setValue(0);
                            databaseReference.child("users").child(uid).child("a-freeplay").setValue("0");
                    ...
                    new Handler().postDelayed(new Runnable() {  //Same as previous
                        ...
                        public void run() {
                            ...
                            float rated = Float.parseFloat(rate);
                            databaseReference.child("users").child(uid).child("a-incorrect").setValue(rated);//Updating incorrect counters
                            databaseReference.child("users").child(uid).child("a-flower").setValue(0);
                            databaseReference.child("users").child(uid).child("a-freeplay").setValue("0");
                            Intent intent = new Intent(A_Activity.this, A_Activity.class);
                            startActivity(intent);
                    ...
                    Log.d("accu", accuracyInfo);
                            databaseReference.child("users").child(uid).child("a").setValue(rated);
                            databaseReference.child("users").child(uid).child("a-flower").setValue(1);
                    databaseReference.child("users").child(uid).child("a-freeplay").setValue("1");//We save the accuracy rate for that letter in the db
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    dialogYes.dismiss(); //Same logic as above we show dialog for 6 seconds

                                    Intent intent = new Intent(A_Activity.this, Difficult_Activity.class);
                                    startActivity(intent);
                                    finish();
                                }
                            }, 6000);
