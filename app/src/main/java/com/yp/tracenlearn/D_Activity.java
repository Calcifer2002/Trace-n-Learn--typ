...
public class D_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DCustomView customDCanvas = findViewById(R.id.customDCanvas); // Drawing canvas
        ...
        MediaPlayer mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.d);
        ...
        dialogNo = new Dialog(D_Activity.this);
        dialogYes = new Dialog(D_Activity.this);
        dialogNoMany = new Dialog(D_Activity.this);
        dialogNoSlow = new Dialog(D_Activity.this);
                    ...
                    if (child instanceof ImageView) {
                        ...
                        child.setOnClickListener(new View.OnClickListener() { // We wait for click
                            ...
                            public void onClick(View v) {
                                customDCanvas.setStrokeColor(getColorForIndex(colorIndex)); // Get the respective colour for that index
                            }
                        });
                    }
                }
        ...
        */
        customDCanvas.setOnNoStrokesDetectedCallback(new DCustomView.NoStrokesCallback() {
            @Override
            public void onNoStrokesDetected(String accuracyInfo) {
                if (freePlay && accuracyInfo.toLowerCase().contains("no")) {     //When in freeplaymode and the letter is not traced properly
                    ...
                    databaseReference.child("users").child(uid).child("d-freeplay-incorrect").setValue(rated); //Keeping track of wrong letter rate
                    databaseReference.child("users").child(uid).child("d-freeplay-flower").setValue(0); //To track flower counter- wrong trace so we set it to 0
                    ...
                    String rate = accuracyInfo.substring(colonIndex + 1, percentIndex).trim();
                    float rated = Float.parseFloat(rate);
                    databaseReference.child("users").child(uid).child("d-freeplay-correct").setValue(rated);//Keeping track of correct letter rate
                    databaseReference.child("users").child(uid).child("d-freeplay-flower").setValue(1); //To track flower counter- correct trace so we set it to 1
                    ...
                    new Handler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            databaseReference.child("users").child(uid).child("d-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("d-attempted").setValue(1);
                            Intent intent = new Intent(D_Activity.this, D_Activity.class);
                    ...
                    new Handler().postDelayed(new Runnable() {  //Same as previous
                        ...
                        public void run() {
                            ...
                            float rated = Float.parseFloat(rate);
                            databaseReference.child("users").child(uid).child("d-incorrect").setValue(rated);//Updating incorrect counters
                            databaseReference.child("users").child(uid).child("d-flower").setValue(0);
                            databaseReference.child("users").child(uid).child("d-freeplay").setValue("0");
                            databaseReference.child("users").child(uid).child("d-attempted").setValue(1);
                            Intent intent = new Intent(D_Activity.this, D_Activity.class);
                    ...
                    databaseReference.child("users").child(uid).child("d-freeplay").setValue("1");
                    databaseReference.child("users").child(uid).child("d-attempted").setValue(1);//We save the accuracy rate for that letter in the db
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            dialogYes.dismiss(); //Same logic as above we show dialog for 6 seconds

                            Intent intent = new Intent(D_Activity.this, Difficult_Activity.class);
                            startActivity(intent);
                            finish();
                        }
                    }, 6000);
